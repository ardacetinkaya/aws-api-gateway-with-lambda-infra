...
RUN dotnet restore "HelloLambda.v2/HelloLambda.v2.csproj"

WORKDIR "/src/HelloLambda.v2"
COPY . .
RUN dotnet build "HelloLambda.v2.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "HelloLambda.v2.csproj" \
...
Execute unit tests
```
    cd "HelloLambda.v2/test/HelloLambda.v2.Tests"
    dotnet test
```

Deploy application
```
    cd "HelloLambda.v2/src/HelloLambda.v2"
