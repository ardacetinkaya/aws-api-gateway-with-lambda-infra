...
RUN dotnet restore "HelloLambda.v3/HelloLambda.v3.csproj"

WORKDIR "/src/HelloLambda.v3"
COPY . .
RUN dotnet build "HelloLambda.v3.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "HelloLambda.v3.csproj" \
...
Execute unit tests
```
    cd "HelloLambda.v3/test/HelloLambda.v3.Tests"
    dotnet test
```

Deploy application
```
    cd "HelloLambda.v3/src/HelloLambda.v3"
