...
RUN dotnet restore "HelloLambda.v4/HelloLambda.v4.csproj"

WORKDIR "/src/HelloLambda.v4"
COPY . .
RUN dotnet build "HelloLambda.v4.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "HelloLambda.v4.csproj" \
...
Execute unit tests
```
cd "HelloLambda.v4/test/HelloLambda.v4.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "HelloLambda.v4/src/HelloLambda.v4"
